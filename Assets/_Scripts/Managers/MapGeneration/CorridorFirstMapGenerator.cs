...
public class CorridorFirstMapGenerator : SimpleRandomWalkMapGenerator
{
    ...
    private void CreateCorridors(HashSet<Vector2Int> floorPositions, HashSet<Vector2Int> potentialRoomPositions)
    {
        ...
        potentialRoomPositions.Add(currPosition);

        for (int i = 0; i < corridorCount; i++)
        {
            var corridor=ProceduralGeneratedAlgorithms.RandomCorridor(currPosition,corridorLength);
            currPosition=corridor[corridor.Count-1];
            potentialRoomPositions.Add(currPosition);
